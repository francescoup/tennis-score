<template>
  <div class="max-w-sm mx-auto p-4 bg-gray-900 rounded-lg shadow-md">
    <h2 class="text-2xl text-center font-semibold mb-4">Sistema di Punteggi Tennis</h2>
    <!-- Punteggio dei giochi -->
      <PointScore :playerOnePoint='player1DisplayScore' :playerTwoPoint='player2DisplayScore'/>
    <!-- Punteggio dei set -->
    ...
    <div class="mb-1 grid grid-cols-2 gap-1">
      <Button @handler="increaseScore(1)" text="player 1"/>
      <Button @handler="increaseScore(2)" text='Player 2'/>
    </div>

    <!-- Pulsante per resettare il punteggio -->
     <div class=" grid grid-cols-2 gap-1">
      <Button @handler="resetScore" text="reset"/>
      <Button @handler="undoAction" text="undo"/>
     </div>
  </div>
...
import PointScore from './components/molecules/PointScore.vue';
import SetsScore from './components/molecules/SetsScore.vue';
...
const player1DisplayScore = computed(() => tieBreack.value ? player1Score.value : scoreMap[player1Score.value]);
const player2DisplayScore = computed(() => tieBreack.value ? player2Score.value : scoreMap[player2Score.value]);
// active user
...
const increaseScore = (player) => {
  ...
  if (player === 1 && (tieBreack.value === true ? player1Score.value <= 100 : player1Score.value < 5)) {
    isDeuce.value = false
    player1Score.value++;
  }
  if (player === 2 && (tieBreack.value === true ? player2Score.value <= 100 : player2Score.value < 5)) {
    isDeuce.value = false
    player2Score.value++;
  }

  // Controllo per la vittoria di un gioco (4 punti o 7 punti in caso di tiebrack)
  if ((tieBreack.value === true ? player1Score.value >= 7 : player1Score.value >= 4) && player1Score.value - player2Score.value >= 2) {
  ...
  } else if ((tieBreack.value === true ? player2Score.value >= 7 : player2Score.value >= 4) && player2Score.value - player1Score.value >= 2) {
    player1Score.value = 0;
    player2Score.value = 0;
    player2Game.value++;
  ...
  }else if(tieBreack.value === false && player1Score.value >= 3 && player1Score.value === player2Score.value){
    isDeuce.value = true
...
const checkForSetWinner = (player) => {
  ...
  if (tieBreack.value === true ? player === 1 && player1Game.value === 7 : player === 1 && player1Game.value >= 6 && player1Game.value - player2Game.value >= 2) {
    player1Sets.value++;
    storeSets(player)
    player1Score.value = 0;
    player2Score.value = 0;
  ...
  } else if (tieBreack.value === true ? player === 2 && player2Game.value === 7 : player === 2 && player2Game.value >= 6 && player2Game.value - player1Game.value >= 2 ) {
    ...
    player2Score.value = 0;
    tieBreack.value = false
  }else if(player1Game.value === 6 && player2Game.value === 6 && player1Game.value === player2Game.value){
...
function storeSets(player){
  ...
  if((player1Sets.value === 1 &&  player2Sets.value === 0) || (player2Sets.value === 1 && player1Sets.value === 0 ) ){
    player1Set1.value = player1Game.value
    player2Set1.value = player2Game.value
  }else if((player1Sets.value === 1 && player2Sets.value === 1) || (player1Sets.value === 2 &&  player2Sets.value === 0) || (player2Sets.value === 2 && player1Sets.value === 0 ) ){
